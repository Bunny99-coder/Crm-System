// Replace the entire contents of your internal/service/lead_service.go file.
package service
...
func NewLeadService(lr *postgres.LeadRepo, cr *postgres.ContactRepo, ur *postgres.UserRepo, pr *postgres.PropertyRepo, logger *slog.Logger) *LeadService {
	return &LeadService{leadRepo: lr, contactRepo: cr, userRepo: ur, propertyRepo: pr, logger: logger}
}
...
func (s *LeadService) GetLeadByID(ctx context.Context, id int) (*models.Lead, error) {
	...
	if lead == nil { return nil, fmt.Errorf("lead with ID %d not found", id) }
	return lead, nil
}
func (s *LeadService) UpdateLead(ctx context.Context, id int, l models.Lead) error {
	...
	if err != nil {
		if err == sql.ErrNoRows { return fmt.Errorf("lead with ID %d not found during update", id) }
		return err
	}
	return nil
}
func (s *LeadService) DeleteLead(ctx context.Context, id int) error {
	...
	if err != nil {
		if err == sql.ErrNoRows { return fmt.Errorf("lead with ID %d not found", id) }
		return err
	}
	return nil
}