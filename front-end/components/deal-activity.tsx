...
import { Clock, FileText, CheckSquare, Calendar, MessageSquare } from "lucide-react"
...
export function DealActivity({ dealId }: DealActivityProps) {
  ...
  const loadActivity = async () => {
    try {
      setIsLoading(true)
      const activityData = await api.getActivities("deal", dealId)
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Recent activity and updates for this deal</CardDescription>
      </CardHeader>
      <CardContent>
        {activities.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-muted-foreground">
              No activity yet. Activity will appear here as you work on this deal.
            </p>
          </div>
        ) : (
